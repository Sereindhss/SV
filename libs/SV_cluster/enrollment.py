...
def enroll(feature, K, L, M, public_key):
    ...
    # 计算压缩整数 C_f 的比特数，并转换为千比特（Kb）
    c_f_bits = C_f.bit_length()
    c_f_kb = c_f_bits / 1024  # 注意：这是千比特，不是字节
...
def main(K, L, M, feature_list, folder, public_key):
    ...
    for i, feature in enumerate(features):
        ...
        c_f_kb_list.append(durations[2])  # 记录压缩整数大小
        if i % 1000 == 0:
            print('{}/{}'.format(i, n))
    ...
    # --- 新增内存与存储开销计算 ---
    # Linux 下 ru_maxrss 的单位是 KB，除以 1024 转换为 MB
    ...
    print('[Metrics] Template File Size (单个特征模板大小): {:.2f} KB\n'.format(sample_file_size))
    print('[Metrics] Average Compressed Integer Size (压缩整数平均大小): {:.2f} Kb'.format(avg_cf_kb))
