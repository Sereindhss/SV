...
parser.add_argument('--public_key', default='libs/SV_DJ_cluster/keys/publickey',
                    type=str)
...
def enroll(feature, K, L, M, s, public_key, R_i):
    from libs.SV_DJ_cluster.enrollment import enroll as dj_enroll
    # dj_enroll in enrollment.py returns [c_f, C_tilde_f], [duration_plain, duration_cypher, c_f_kb]
    return dj_enroll(feature, K, L, M, public_key, R_i)
...
#--tihuan--
    ...
    pubkey_path = '{}_{}_s{}.npy'.format(args.public_key, args.key_size, args.s)
    publickey = np.load(pubkey_path, allow_pickle=True)[0]

    from libs.SV_DJ_cluster.enrollment import offline_precompute_Ri
    print('[ClusterIndex] Precomputing R_i for {} cluster centers...'.format(n_clusters))
    R_pool, offline_time = offline_precompute_Ri(n_clusters, publickey)

    start = time.time()
    for c in range(n_clusters):
        R_i = R_pool.pop()
        result, durations = enroll(centers[c], K, L, M, args.s, publickey, R_i)
    ...
    total_build_time = cluster_duration + enroll_duration + offline_time
    print('  Center encryption completed in {:.2f}s (offline: {:.2f}s)'.format(enroll_duration, offline_time))
