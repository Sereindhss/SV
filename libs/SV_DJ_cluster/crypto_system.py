...
def build_arg_parser():
    ...
    parser.add_argument(
        '--crt_decrypt',
        action='store_true',
        help='Use CRT-accelerated threshold decrypt (requires libs/SV_DJ_cluster/keys/factors_<key_size>_s<s>.json from genkey)',
    )
    parser.add_argument(
        '--jobs',
        type=int,
...
KEYS_DIR = 'libs/SV_DJ_cluster/keys'
...
def calculate_sim(c_x, c_y, C_tilde_x, C_tilde_y, K, L, M, priv=None, crt_tuple=_CRT_UNSET):
    ...
    if ct is not None:
        from libs.SV_DJ_cluster.dj_crt_decrypt import decrypt_encrypted_number_crt
...
def main(folder, pair_list, score_list, K, L, M, s, key_size, feat_list):
    ...
    pub_key_path = 'libs/SV_DJ_cluster/keys/publickey_{}_s{}.npy'.format(key_size, s)
    priv_key_path = 'libs/SV_DJ_cluster/keys/privatekey_{}_s{}.npy'.format(key_size, s)
...
if __name__ == '__main__':
    ...
    if args.genkey == 1:
        from libs.SV_DJ_cluster.dj_crt_decrypt import keygen_save_factors, factors_json_path
    ...
    if args.crt_decrypt:
        from libs.SV_DJ_cluster.dj_crt_decrypt import load_factors_json, verify_crt_matches_reference, factors_json_path
        ...
        if fac is None:
            raise SystemExit(
                ...
                'Regenerate with: python3 libs/SV_DJ_cluster/crypto_system.py --genkey 1 --key_size {} --s {}'.format(
