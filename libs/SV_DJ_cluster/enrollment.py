...
parser.add_argument(
    '--r_pool_cache_dir',
    default='libs/SV_DJ_cluster/cache',
    type=str,
...
def enroll(feature, K, L, M, public_key, precomputed_R_i):
    ...
    # 1. 计算极小明文的高次幂：(n+1)^m mod n^{s+1} (二项式展开优化)
    C_f_mpz = mpz(C_f)
    if s == 1:
        # 当 s=1 时，复杂度从 O(log(m)) 降维到 O(1) 的基础乘法
        gm_m = gmpy2.f_mod(gmpy2.add(1, gmpy2.mul(C_f_mpz, n_mpz)), n_s_plus_1)
    else:
        # s > 1 时，使用二项式展开的前 s 阶项
        gm_m = mpz(1)
        term = mpz(1)
        n_power = mpz(1)
        for j in range(1, s + 1):
            term = gmpy2.divexact(gmpy2.mul(term, C_f_mpz - j + 1), j)
            n_power = gmpy2.mul(n_power, n_mpz)
            gm_m = gmpy2.add(gm_m, gmpy2.mul(term, n_power))
        gm_m = gmpy2.f_mod(gm_m, n_s_plus_1)
...
def main(K, L, M, s, key_size, feature_list, folder, public_key_path, use_r_pool_cache, r_pool_cache_dir):
    ...
    if int(publickey.s) != int(s):
        raise SystemExit(
            'Public key s={} does not match --s {}. Remove stale keys under libs/SV_DJ_cluster/keys/ '
            'or regenerate with: python3 libs/SV_DJ_cluster/crypto_system.py --genkey 1 --key_size {} --s {}'.format(
